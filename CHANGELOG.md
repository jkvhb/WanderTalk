...
### Added
- 展示页转场升级为受限动效语法：AI 可在路线绽放、方向擦拭、照片级联、柔和叠化、层叠展开和章节滑入之间选择入口，并组合锚点、方向、强度、布局与退出方式；模型输出始终经白名单归一化。
- 无图但有旁白的节点也可进行 AI 编排，自动使用文字优先的安全降级；系统“减少动态效果”偏好统一回落为柔和叠化。

### Fixed
- 修复展示页根元素始终使用圆形裁切、导致 AI 编排视觉上没有变化的问题；旧配置仍兼容原圆形揭幕。
- 修复服务端 `{ index, config }` 编排响应在工作室被当作扁平配置读取、从而丢失 AI 结果的问题。
