...
### Added
- 固定 318 权威底稿升级为九天、46 个不重复主线地点；补入 G318/G248 交叉口（营官村）和尼玛贡神山观景台，并保存可追溯地点证据。
- 新增逐段路线发布门槛：45/45 路段必须全部完成，且不得出现缺段、两点直线、终点偏移或未复核异常绕行，才能进入文案、配图、播报和动画生产。
- 新增跨 318、317、格聂南线和成都城市路线的精准搜图基准，图片先通过地点身份硬校验，再评估视觉内容；同类但不同地点的图片不得自动入库。
- 设置页新增浏览器本地 Kimi Key 配置，并保留服务端环境变量兜底；浏览器密钥不会写入项目文件。
- 节点讲解改为五种地图主导预设：左右素材轨、上下胶片带和单图特写轨；地图可见面积始终不低于约 65%。
- 节点名称、海拔地形标尺、路线序号成为地图上的主信息；图片按旁白顺序在边缘素材区出现。
- 新增本地安全布局解析器：自动避开节点与路线、限制同类预设连续出现，并在无安全位置时回退纯地图信息。
- AI 编排改为只输出连续叙事、并列介绍、单一重点、图片顺序和讲解节拍；具体预设与坐标全部由本地程序决定。

### Changed
- 固定 318 路线数据版本升级为 `2026-08-02-v1`；旧版用户内容按 `placeId` 保留，新增节点自动补齐，受影响日期清除旧路线并要求重新计算。
- 固定 318 已从权威底稿开始完成路线、文案、配图、配音、动效和动画预览的用户整链路验收，并冻结为视觉样章的输入基线。
- 删除圆形揭幕、全屏图片、自由漂浮卡片及其旧运行时代码；旧配置自动迁移为新语义格式，不再触发旧效果。
- 到站镜头使用固定区域级缩放和 `easeTo` 平滑居中，避免弧线缩放穿入 3D 地形；小车在讲解期间停留于当前节点。
- 自动配图会保存图片标题、标签和简介，供 AI 结合旁白判断图片讲解顺序。

### Fixed
- 两张主图可在同一个特写位置按旁白节拍替换，不再丢失第二张图片。
- Vitest 排除嵌套工作树，避免测试数量被重复收集。
- 跨天路线衔接点不再被误显示成待配图讲解节点，自动配图后的缩略图无需刷新即可出现。
- 从分支工作树启动服务时可安全读取主项目根目录的 `.env`，避免误报缺少服务端图片 API Key。

## [0.5.0] - 2026-07-24

### Changed
- 固定 318 预设重整为成都至拉萨九天主线：补齐路线点、跨日地点身份、可选支线标记与高德地点检索来源；理塘、米堆冰川等重复讲解风险由发布校验与播放器双重拦截。
- 新增地点核验表，记录每个主线节点的高德检索证据、WGS-84 坐标和逐日驾车求解结果。
