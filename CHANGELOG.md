...
第四阶段续：AI 自动配图（Pixabay，4d）+ 展示页编排动效（4e，动效词汇表 + LLM 配参）。

### Added（Phase 4e 编排动效）
- 视频工作室「AI 编排动效（有图节点）」：DeepSeek 依旁白文本为每个"有旁白且有图"的节点个性化生成动效配置（tempo 节奏/相位表/待机强度），按 narrationHash 幂等——旁白没改的节点重跑自动跳过
- 后端 `POST /api/choreography`：批量生成编排配置；系统提示词内嵌动效词汇表语义，LLM 只输出受限 JSON（选词+调参+分相位），绝不输出代码/CSS
- 动效词汇表 `src/utils/choreography.js`：normalize（清洗非法配置）/default（均分相位兜底）/compile（配置+seed → 每卡漂移/呼吸/入场参数）纯函数；`src/utils/rand.js` mulberry32——seed=旁白哈希，同节点每次播放动效完全一致（将来导出视频不跑偏）
- 展示页卡片编排（≥2 图节点）：卡片在中上/右侧安全区散落，错峰滑入、缓慢漂移+缩放呼吸（CSS 动画自走），随旁白进度切换焦点卡（放大置顶，其余退后微暗），pulse 相位一次性强调弹跳；1 图节点全屏铺底仅微呼吸（≤2%）；无配置节点渲染与之前完全一致
- `sampleAt` 的 showcase 新增 `narrationFrac`（旁白进度比例 0~1），驱动相位切换
...
### 不做（本期，见 docs/specs/ 下 4d/4e 两份设计文档）
- 4d：改旁白文本（"边写边搜改文本"归 Phase 5 编排层）、逐节点手动重搜按钮、LLM 逐图裁判、图片来源署名 UI
- 4e：逐句节拍同步（edge-tts 词级时间戳，V2 连带逐句字幕）、文字块动效、卡片交互（点击放大等）、导出渲染器对 CSS 待机动画的逐帧重放（导出阶段再统一处理）
